...
//    }
    ...
    // ====== OVERLOAD cho Users ======
    public String generateToken(com.microsoft.hsf302_project.entity.Users user) {
        // mặc định role = USER nếu không có
        String role = "USER";
        return generateTokenFromSubjectAndRole(user.getUsername(), role);
    }

    public String generateRefreshToken(com.microsoft.hsf302_project.entity.Users user) {
        String role = "USER";
        return generateRefreshFromSubject(user.getUsername());
    }

    // === Helpers để tái dùng logic ký (không đụng vào code cũ) ===
    private String generateTokenFromSubjectAndRole(String subject, String role) {
        JWSHeader jwsHeader = new JWSHeader(JWSAlgorithm.HS512);
        JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder()
                .issuer("swp391.com")
                .subject(subject)
    ...
    private String generateRefreshFromSubject(String subject) {
        JWSHeader jwsHeader = new JWSHeader(JWSAlgorithm.HS512);
        JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder()
                .issuer("swp391.com")
                .subject(subject)
